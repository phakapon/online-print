...
type PaginationBuilderRepository interface {
	BuildProductsorderMetadata(*http.Request) (*Metadata, error)
}
...
func NewPaginationBuilderRepository(productsorderRepository ProductsorderRepository) *paginationBuilderRepositoryImpl {
	return &paginationBuilderRepositoryImpl{productsorderRepository}
}
...
func (p *paginationBuilderRepositoryImpl) BuildProductsorderMetadata(r *http.Request) (*Metadata, error) {
	page, limit, offset := p.BuildPagination(r)

	total, err := p.productsorderRepository.Count()
	if err != nil {
		return nil, err
	}
